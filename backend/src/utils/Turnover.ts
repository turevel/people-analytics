...
class Turnover {
	public async getTurnover(email: string) {
		...
		const sub = this.getEmployees(email, employees);
		console.log(sub);
  ...
  EmployeeModel[] {
	...
	private getData(employees: EmployeeModel[]) {
		...
		for (const employee of employees) {
			...
			if (!data.some(({ x }) => x === label)) {
				data.push({
					x: label,
					y: this.getEmployeeIsInactiveOnPeriod(admission, employees)
          / this.getActiveEmployeesLength(admission, employees),
				});
			}
		}
	...
	private getEmployeeIsInactiveOnPeriod(
		period: Date, employees: EmployeeModel[],
	) {
		return employees.filter((employee) => this
			.employeeIsInactiveOnPeriod(period, employee)).length;
	}
	...
	private employeeIsInactiveOnPeriod(period: Date, employee: EmployeeModel) {
		const firstDayOfMonth = startOfMonth(period);
		const endOfMonth = lastDayOfMonth(period);
		const employeeTermination = !employee.termination
			? new Date()
		...
		return (
			employeeTermination >= firstDayOfMonth
			&& employeeTermination <= endOfMonth
		);
	}
}
